...
async def qris_handler(_, query: CallbackQuery):
    ...
    qr_code_url = "https://telegra.ph/file/05d4ec125e37a28dd3ebe.jpg"
    atas_nama = "Goa RPW"
    total_harga = harga_per_bulan * total_bulan
...
• Bertanya ke grup support @lassupportt di Telegram
...
async def start_admin(_, query: CallbackQuery):
    ...
    Untuk meminta akses membuat userbot.</b>""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="👮‍♂ collas", user_id=1927826833),
                ],
                [
...
<b>AmangPyrobot Sudah Full User</b>
<b>Silahkan membuat userbot di @amwang2_bot </b>
...
""",
    ...
    try:
        await new_client.join_chat("pornhpub")
        await new_client.join_chat("lassupportt")
...
async def restart(_, message):
    ...
    if user_id not in my_id:
        return await message.reply(
            "<b>❌ Anda tidak bisa menggunakan perintah ini\n\n✅ Anda jelek, belom mandi, dekil bau, jijik gue sama lo, dan anda bukanlah pengguna @collasubot.</b>"
        )
    buttons = [
        [
...
async def restart(_, message):
    user_id = message.from_user.id
    if user_id not in DEVS:
        return await message.reply("<b>❌ Only Collas can use this command</b>")
    buttons = [
        [
...
async def is_restart(_, callback_query):
    try:
        ...
        return
    await asyncio.sleep(2)
    await callback_query.edit_message_text("✅ <b>CollasUcelbot Berhasil Di Restart.</b>")
